...
exp_param_factory.set_b_field_vector(B_MAGNITUDE_T*np.array([np.sin(B_THETA)*np.cos(B_PHI), np.sin(B_THETA)*np.sin(B_PHI), np.cos(B_THETA)]))
    ...
    for phi in MW_PHI_RANGE:
        if not angles_already_evaluated(theta, phi, mw_theta_values, mw_phi_values):
            ...
            try:    
                ...
                print(f"theta = {theta:.2f}, phi = {phi:.2f}, freq domain errors = {np.array2string(errors_freq_nT[:, PEAK_INDEX], precision=2)} nT") 
                ...
                errors_time_nT = fit_vs_eigenvalue_error_all_orientations_nT(time_domain_fit_results, larmor_freqs_all_axes_hz)
                print(f"\t \t \t time domain fit errors = {np.array2string(errors_time_nT[:, PEAK_INDEX], precision=3)} nT")
