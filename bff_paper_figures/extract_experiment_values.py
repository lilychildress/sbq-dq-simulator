...
def find_rabi_frequencies_from_signal(sq_cancelled_signal, inner_product_settings, rabi_freq_range_to_probe_hz=np.linspace(50e6, 100e6, 101), height_factor = 0.75, prominence_factor = 0.75, ordering=[0,1,3,2], use_time_domain_summation_for_initial_guesses=True, ramseyfreqs= np.linspace(0, 8.5e6, 11)):
    ...
    if use_time_domain_summation_for_initial_guesses:
        ...
        ramsey_summed = np.array([np.sum(inner_product_sinusoid(np.cos, rabi_freq, inner_product_settings.mw_pulse_durations_s, window*np.transpose(sq_cancelled_signal), axis=1)) for rabi_freq in rabi_freq_range_to_probe_hz])
    else:
        dtft = np.array([[double_cosine_inner_product(sq_cancelled_signal, rabi_hz, ramsey_hz, inner_product_settings) for ramsey_hz in ramseyfreqs] for rabi_hz in rabi_freq_range_to_probe_hz])
