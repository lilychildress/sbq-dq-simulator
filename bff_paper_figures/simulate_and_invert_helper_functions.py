...
def double_cosine_inner_product_fit_inversion(
    ...
    for orientation in NVOrientation:
        ...
        first_attempt_peaks = extract_fit_centers(
            fit_constrained_hyperfine_peaks(ramsey_freq_range_init_guess_hz, cos_cos_inner_prod, t2star_s,constrain_same_width=constrain_same_width, allow_zero_peak=allow_zero_peak, constrain_hyperfine_splittings=constrain_hyperfine_splittings)
        )
        ...
        fit_result = fit_constrained_hyperfine_peaks(
            ramsey_freq_range_constrained_hz, cos_cos_inner_prod, t2star_s, constrain_same_width=constrain_same_width, allow_zero_peak=allow_zero_peak, constrain_hyperfine_splittings=constrain_hyperfine_splittings,
        )
        fit_results.append(fit_result)

    return fit_results

def get_ramsey_freq_guesses_quickly(sq_cancelled_signal, rabi_frequencies, inner_product_settings, RAMSEY_FREQ_RANGE_INITIAL_GUESS_HZ, height_factor=0.75, prominence_factor=0.75) -> NDArray:
...
def time_domain_fit_inversion(sq_cancelled_signal: NDArray, rabi_frequencies: NDArray, inner_product_settings: InnerProductSettings, ramsey_freq_guesses_all_orientations: NDArray, t2star_s:float, fix_phase_to_zero:bool=False, constrain_same_decay:bool=False, constrain_hyperfine_freqs:bool=False):
