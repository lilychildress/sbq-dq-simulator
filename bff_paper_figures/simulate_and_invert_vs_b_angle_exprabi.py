...
from bff_paper_figures.extract_experiment_values import find_rabi_frequencies_from_signal
...
RUN_LABEL = f"rabiexp_b_{B_MAGNITUDE_T * T_TO_UT:.0f}_ut_t2s_{T2STAR_S * 1e6:.0f}_us_fine_3us_ramsey_800ns_rabi"
...
for theta in np.linspace(B_THETA_START, B_THETA_STOP, B_THETA_N):
    ...
    for phi in np.linspace(B_PHI_START, B_PHI_STOP, b_phi_n):
        if not angles_already_evaluated(theta, phi, theta_values, phi_values):
            ...
            ideal_rabi_frequencies = get_ideal_rabi_frequencies(exp_param_factory.get_experiment_parameters())
            print(ideal_rabi_frequencies)
            data_rabi_frequencies = find_rabi_frequencies_from_signal(sq_cancelled_signal, inner_product_settings)
            print(data_rabi_frequencies)
            try:
                # frequency domain inversion
                peakfit_results = freq_domain_inversion(
                    sq_cancelled_signal,
                    data_rabi_frequencies,
