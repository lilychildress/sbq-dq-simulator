...
def set_up_three_peak_model(zero_peak_amplitude:float, signal_peak_amplitude:float, rightmost_peak_location:float, t2star_s:float) -> tuple:
...
def fit_constrained_hyperfine_peaks(ramsey_freqs_range_hz: NDArray, double_inner_product_cos_cos: NDArray, t2star_s:float=2e-6, height_factor:float=1/3, prominence_factor:float=1/3) -> ModelResult:
...
def plot_fit_vs_inner_product(ramsey_freqs_range_hz:NDArray, double_inner_product_cos_cos:NDArray, fit_result:ModelResult) -> None:
    fit_peaks = extract_fit_centers(fit_result)
    plt.vlines(HZ_TO_MHZ*fit_peaks, [max(double_inner_product_cos_cos)],[min(double_inner_product_cos_cos)], label = "fitted peaks", color="red")
