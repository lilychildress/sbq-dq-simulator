...
# Returns something that is equal to zero when theta and phi correspond to an axial field projection of
# half of the hyperfine splitting
def half_hyperfine(theta: float, phi: float) -> float:
    return (-np.cos(theta) - np.cos(phi) * np.sin(theta) + np.sin(theta) * np.sin(phi)) / np.sqrt(3) - f_h / (
        2 * B_MAGNITUDE_T * gammab
    )
...
def get_rms_error_within_range(
    theta_range: list,
    phi_range: list,
    phi_values_total: NDArray,
    theta_values_total: NDArray,
    errors_vs_b_nt_total: NDArray,
    orientation: int,
) -> float:
    masks = np.array(
        [
            theta_values_total * 360 / (2 * np.pi) < theta_range[1],
            theta_values_total * 360 / (2 * np.pi) > theta_range[0],
            phi_values_total * 360 / (2 * np.pi) < phi_range[1],
            phi_values_total * 360 / (2 * np.pi) > phi_range[0],
        ]
    )
...
def plot_errors_all_orientations(
    theta_values_total: NDArray,
    phi_values_total: NDArray,
    errors_vs_b_nt_total: NDArray,
    plot_half_hyperfine: bool = False,
    log_plot: bool = True,
    vmin: float = 0.01,
    vmax: float = 300,
) -> None:
    ...
    errors = [
        get_rms_error_within_range(
            theta_ranges[i],
            phi_ranges[i],
            np.array(phi_values_total),
            np.array(theta_values_total),
            np.array(errors_vs_b_nt_total),
            i,
        )
        for i in range(4)
    ]
    ...
    for i, ax in enumerate(axes.flat):
        im = ax.tripcolor(
            np.array(phi_values_total) * RAD_TO_DEGREE,
            np.array(theta_values_total) * RAD_TO_DEGREE,
            np.abs(np.array(errors_vs_b_nt_total)[:, i]),
            cmap="inferno",
            norm=plotstyle,
            vmin=vmin,
            vmax=vmax,
        ...
        if i == 1:
            ...
            if plot_half_hyperfine:
                ax.plot(
                    PHI_RANGE_HALF_HYPERFINE * RAD_TO_DEGREE,
                    theta_hh_rad * RAD_TO_DEGREE,
                    color="white",
                    linestyle="dashed",
                )
                ax.plot(
                    PHI_RANGE_HALF_HYPERFINE * RAD_TO_DEGREE,
                    theta_hh_rad_low * RAD_TO_DEGREE,
                    color="white",
                    linestyle="dashed",
                )
