...
from bff_paper_figures.fit_inner_product_vs_ramsey import fit_constrained_hyperfine_peaks, plot_fit_vs_inner_product, fit_vs_eigenvalue_error_nT, extract_fit_centers
...
B_FIELD_VECTOR_T = [1e6/(gammab*np.sqrt(3)) * x for x in [1, 4, 1.2]]
...
for i, orientation in enumerate(NVOrientation):
    ...
    cos_cos_inner_prod=double_cosine_inner_product_vs_ramsey(sq_cancelled_signal, rabi_frequencies[orientation], RAMSEY_FREQ_RANGE_INITIAL_GUESS_HZ, inner_product_settings)
    first_attempt_peaks = extract_fit_centers(fit_constrained_hyperfine_peaks(RAMSEY_FREQ_RANGE_INITIAL_GUESS_HZ, cos_cos_inner_prod, T2STAR_S))
    ...
    cos_cos_inner_prod=double_cosine_inner_product_vs_ramsey(sq_cancelled_signal, rabi_frequencies[orientation], ramsey_freq_range_constrained_hz, inner_product_settings)
    fit_result = fit_constrained_hyperfine_peaks(ramsey_freq_range_constrained_hz, cos_cos_inner_prod, T2STAR_S)
    ...
    plt.title(f"Axis: {np.array2string(np.sqrt(3)*NVaxes_100[orientation], precision=0)}, Rabi: {rabi_frequencies[orientation]*1e-6:.1f} MHz\nErrors: {np.array2string(errors_nT, precision=2)} nT")
plt.tight_layout() 

plt.suptitle(f"B field: {np.array2string(T_TO_UT*np.array(B_FIELD_VECTOR_T), precision=1)} uT, Rabi window: {inner_product_settings.rabi_window}, Ramsey window: {inner_product_settings.ramsey_window}, {"mean subtracted" if inner_product_settings.subtract_mean else ""}, {"using effective Rabi" if inner_product_settings.use_effective_rabi_frequency else ""}")
plt.show()