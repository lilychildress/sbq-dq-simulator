...
# the bff_experiment/extract_inner_product_peaks.py script, which is run on the
...
ramsey_frequencies_hz = np.loadtxt(base_path+"ramsey_frequencies_hz.txt")
field_projections_t = np.loadtxt(base_path+"field_projections_t.txt")
voltages = np.loadtxt(base_path+"voltages.txt")
#########################################################################
...
for i, rabi_frequency_fixed_orientation in enumerate(np.transpose(rabi_frequencies_hz)):
    plt.scatter(voltages, HZ_TO_MHZ*rabi_frequency_fixed_orientation, color=COLORS[i], marker=MARKERS[i], s=20, linestyle="")
...
for i, field_projection_fixed_orientation in enumerate(np.transpose(field_projections_t)):
    ...
    plt.scatter(voltages, T_TO_UT*field_projection_fixed_orientation, color=COLORS[i], marker = MARKERS[i], s=20, linestyle="")
#plt.gca().set_aspect(1)
plt.xlabel("Coil voltage (V)")
